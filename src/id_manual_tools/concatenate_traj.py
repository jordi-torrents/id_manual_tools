...
video = "20220712_0192"
sessions_name = "010203040192", "050607080192", "09101112130192"
...
for ax_i, session in enumerate(sessions[1:]):
    matcher = np.load(
        session + "matching_results/" + sessions_name[0] + "-.npy", allow_pickle=True
    ...
    assert all([i == j for i, j in zip(main_out["setup_points"], data["setup_points"])])
    assert all(
        [
            i == j
            for i, j in zip(main_out["identities_groups"], data["identities_groups"])
        ]
    )
...
# """
# trajectories (16050, 39, 2)
# version 4.0.8
# video_path /home/jordi/GX010128.MP4
# frames_per_second 50
# body_length 69.0
# stats dict_keys(['estimated_accuracy', 'estimated_accuracy_after_interpolation', 'percentage_identified', 'estimated_accuracy_identified'])
# id_probabilities (16050, 39, 1)
# areas (16050, 39)
# setup_points dict_keys(['cantonades'])
# identities_groups dict_keys([])
# """
